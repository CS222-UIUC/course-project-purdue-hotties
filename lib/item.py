...
class Block(Item):
    ...
    def randomize(self, snake_body, items_pos):
        ...
        direction = [1, 0, -1, 0, 1]
        no_wall_zone = [Vector2(snake_head[0] + direction[i], snake_head[1] + direction[i+1]) for i in range(DIRECTION_NUM)]
        for item_p in items_pos:
            no_wall_zone += [Vector2(item_p[0] + direction[i], item_p[1] + direction[i+1]) for i in range(DIRECTION_NUM)]
